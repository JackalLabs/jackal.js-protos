...
export default {
	...
	getters: {
			...
			return state.FidCidAll[JSON.stringify(params)] ?? {}
				...
				getGetPayData: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.GetPayData[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryGetPayData({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryGetPayData( key.address)).data
				
					
				commit('QUERY', { query: 'GetPayData', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryGetPayData', payload: { options: { all }, params: {...key},query }})
				return getters['getGetPayData']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryGetPayData API Node Unavailable. Could not perform query: ' + e.message)
