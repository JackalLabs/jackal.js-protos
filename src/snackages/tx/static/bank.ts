...
export class TxBank implements ITxBank {
  msgMultiSend (data: MsgMultiSend): EncodeObject {
    return { typeUrl: bankTypes.multiSend[0], value: bankTypes.multiSend[1].fromPartial( data ) }
  }
  msgSend (data: MsgSend): EncodeObject {
    return { typeUrl: bankTypes.send[0], value: bankTypes.send[1].fromPartial( data ) }
  }
}
